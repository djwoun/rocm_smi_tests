...
// --- START: AMD SMI Definitions (Copied from gemm.cpp) ---
...
// --- END: AMD SMI Definitions ---


// Structure to pass parameters to the monitoring thread.
struct monitor_params {
    // int EventSet; // Removed PAPI EventSet
    amdsmi_processor_handle gpu_handle; // Added AMD SMI Handle
    FILE *csvFile;
    struct timeval start_time;
    uint32_t monitor_interval_us; // Added Monitoring interval in microseconds
};


// --- Function Prototypes for AMD SMI Collection (Copied from gemm.cpp) ---
...
// --- START: AMD SMI Collection Functions (Copied from gemm.cpp) ---
...
// (Replaced with AMD SMI version from gemm.cpp)
...
// Helper function for cleanup (Adapted from gemm.cpp for GEMV resources)
void cleanup_resources(FILE *csvFile,
                       ...
                       amdsmi_processor_handle *processor_handles, // Pass pointer to processor handles array (from discovery)
                       amdsmi_processor_handle target_gpu_handle // Pass the target handle itself (might be NULL on early exit)
                      )
{
    ...
    // AMD SMI Cleanup
    // processor_handles points to the array allocated during discovery for the *socket*
    // containing the target_gpu_handle. It needs to be freed.
    // If target_gpu_handle is NULL (e.g., error before it was found), processor_handles might be NULL too,
    // or it might point to the handles from the last attempted socket, or it might have been freed already
    // if allocation failed mid-discovery. Checking processor_handles itself is usually sufficient.
    if (processor_handles != NULL) {
        ...
        printf("Processor_handles array was NULL or already freed.\n");
    }


    // Shut down SMI library regardless of whether handles were found/used
...
int main(int argc, char *argv[]) {
    ...
    double *h_A = NULL, *h_B = NULL, *h_C = NULL; // Host memory (A=matrix, B=vector x, C=vector y)
    ...
    // --- Setup Arrays for Cleanup (GEMV specific: A, B(x), C(y)) ---
    // We need pointers for d_A[s], d_B[s], d_C[s] for each stream 's'
    const int NUM_DEVICE_ARRAYS = NUM_STREAMS * 3;
    double *device_arrays_ptrs[NUM_DEVICE_ARRAYS];
    for(int i = 0; i < NUM_DEVICE_ARRAYS; ++i) {
        device_arrays_ptrs[i] = NULL;
    }
    const int NUM_HOST_ARRAYS = 3;
    double *host_arrays_ptrs[NUM_HOST_ARRAYS] = {NULL, NULL, NULL}; // h_A, h_B, h_C
    ...
    // processor_handles will point to the temp_handles of the *correct* socket once found
    processor_handles = NULL;
    ...
    for (uint32_t i = 0; i < socket_count && target_gpu_handle == NULL; ++i) {
        ...
        if (smi_status != AMDSMI_STATUS_SUCCESS || device_count == 0) {
            fprintf(stderr, "Skipping socket %u: Failed to get device count or no devices found (Error: %d)\n", i, smi_status);
        ...
        if (!temp_handles) {
            ...
            // processor_handles should still be NULL here, no need to free it
            free(sockets);
            amdsmi_shut_down(); return -1;
        }
        ...
        if (smi_status != AMDSMI_STATUS_SUCCESS) {
            ...
            free(temp_handles); temp_handles = NULL; // Prevent use below
            continue; // Try next socket
        }

        // Now iterate through devices on *this* socket
        for (uint32_t j = 0; j < device_count; ++j) {
            ...
            if (smi_status == AMDSMI_STATUS_SUCCESS && processor_type == AMDSMI_PROCESSOR_TYPE_AMD_GPU) {
                // Found a GPU, check if it's the one we want
                if (current_gpu_index_counter == target_logical_gpu_index) {
                    target_gpu_handle = temp_handles[j];
                    processor_handles = temp_handles; // **** Assign array pointer for later cleanup ****
                    printf("Found target GPU handle for logical index %u (Socket %u, Device %u).\n", target_logical_gpu_index, i, j);
                    break; // Found target, exit inner loop
                }
                current_gpu_index_counter++; // Increment GPU counter only if it *is* a GPU
            } else if (smi_status != AMDSMI_STATUS_SUCCESS) {
                 fprintf(stderr, "Warning: Could not get processor type for device %u on socket %u (Error: %d)\n", j, i, smi_status);
            }
        } // End device loop for this socket

        // If we finished the device loop for this socket but *didn't* find the target,
        // and we haven't assigned processor_handles (because the target wasn't on this socket),
        // we need to free the temp_handles we allocated just for this socket's iteration.
        if (target_gpu_handle == NULL) {
             if (temp_handles != NULL) { // Check because allocation might have failed earlier
                free(temp_handles);
                temp_handles = NULL;
             }
        }
        // If target_gpu_handle *was* found, processor_handles now points to temp_handles,
        // and we should *not* free temp_handles here. It will be freed in cleanup_resources.
    ...
    if (target_gpu_handle == NULL) {
        ...
        // processor_handles might be NULL or point to the last socket's handles if loop finished without finding
        cleanup_resources(NULL, NULL, NULL, 0, NULL, 0, NULL, 0, processor_handles, NULL); // Pass processor_handles if it's not NULL
        return -1;
    }
    ...
    hipStatus = hipSetDevice(hip_device_index);
     if (hipStatus != hipSuccess) {
         ...
         // Pass processor_handles here because discovery succeeded even if hipSetDevice failed
    ...
    /* Allocate host memory for matrix A, vector x, and vector y with page-locked memory */
    size_t size_A = ((size_t)M_DIM * K_DIM * sizeof(double));
    size_t size_B = ((size_t)K_DIM * sizeof(double));  // Vector x (k x 1)
    size_t size_C = ((size_t)M_DIM * sizeof(double));  // Vector y (m x 1)
    ...
    for (size_t i = 0; i < (size_t)K_DIM; i++) h_B[i] = (double)(i % 100); // Init vector x
    for (size_t i = 0; i < (size_t)M_DIM; i++) h_C[i] = 0.0;                 // Init vector y
    printf("Host data initialized.\n");


    /* Allocate device memory and create streams/events */
    printf("Allocating device memory and creating streams/events...\n");
    for (int s = 0; s < NUM_STREAMS; s++) {
        ...
        device_arrays_ptrs[s*3 + 0] = d_A[s]; // Track for cleanup
        ...
        device_arrays_ptrs[s*3 + 1] = d_B[s]; // Track for cleanup
        ...
        device_arrays_ptrs[s*3 + 2] = d_C[s]; // Track for cleanup

        hipStatus = hipStreamCreateWithFlags(&streams[s], hipStreamNonBlocking);
        if (hipStatus != hipSuccess) { fprintf(stderr, "hipStreamCreate failed stream %d: %s\n", s, hipGetErrorString(hipStatus)); cleanup_resources(NULL, streams, events, s+1, device_arrays_ptrs, NUM_DEVICE_ARRAYS, host_arrays_ptrs, 3, processor_handles, target_gpu_handle); return -1; }
    ...
    for (int s = 0; s < NUM_STREAMS; s++) {
        ...
        if (hipStatus != hipSuccess) { fprintf(stderr, "hipMemcpyAsync d_A[%d] failed: %s\n", s, hipGetErrorString(hipStatus)); cleanup_resources(NULL, streams, events, NUM_STREAMS, device_arrays_ptrs, NUM_DEVICE_ARRAYS, host_arrays_ptrs, 3, processor_handles, target_gpu_handle); return -1; }
    ...
    csvFile = fopen("gemv_monitoring_amdsmi.csv", "w"); // Changed filename
    if (!csvFile) {
        fprintf(stderr, "Failed to open CSV file for writing.\n");
        cleanup_resources(NULL, streams, events, NUM_STREAMS, device_arrays_ptrs, NUM_DEVICE_ARRAYS, host_arrays_ptrs, 3, processor_handles, target_gpu_handle);
        return -1;
    }
    ...
    if (thread_status != 0) {
        ...
        // Pass currently allocated resources to cleanup
        cleanup_resources(csvFile, streams, events, NUM_STREAMS, device_arrays_ptrs, NUM_DEVICE_ARRAYS, host_arrays_ptrs, 3, processor_handles, target_gpu_handle);
        return -1;
    }
    ...
    int threadsPerBlock = 128; // Keep original block size
    // Grid size depends on M_DIM (output vector length)
    int blocksPerGrid = (M_DIM + threadsPerBlock - 1) / threadsPerBlock;
    dim3 gridDim(blocksPerGrid); // 1D grid
    dim3 blockDim(threadsPerBlock); // 1D block


    printf("Launching DGEMV kernel (%d iterations per stream)...\n", ITERATIONS_PER_STREAM);
    usleep(5000000); // Keep original 5s delay before starting work
    ...
    for (int iter = 0; iter < ITERATIONS_PER_STREAM; iter++) {
        for (int s = 0; s < NUM_STREAMS; s++) {
            ...
            hipLaunchKernelGGL(dgemv_kernel, gridDim, blockDim, 0, streams[s],
                               d_A[s], d_B[s], d_C[s],
            ...
            // Record event but don't synchronize yet within the inner loop
        ...
        // Wait for all streams in this iteration to finish *before* the sleep
        for (int s = 0; s < NUM_STREAMS; s++) {
            hipStatus = hipStreamSynchronize(streams[s]);
            if (hipStatus != hipSuccess) {
        ...
        // Keep original 5s delay between iterations
        if (iter < ITERATIONS_PER_STREAM -1) { // Don't sleep after the last iteration
             usleep(5000000);
        }
    } // End iteration loop
    printf("DGEMV kernel executions finished.\n");
    ...
    /* Cleanup resources using the helper function */
    cleanup_resources(csvFile, streams, events, NUM_STREAMS, device_arrays_ptrs, NUM_DEVICE_ARRAYS, host_arrays_ptrs, 3, processor_handles, target_gpu_handle);
